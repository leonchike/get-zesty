...
import { getAviationWeather, formatAviationWeatherMarkdown, createAviationWeatherErrorResponse } from "./metar-api.js";
import { resolveSimBriefUserId } from "../config/allowed-users.js";
...
const GetAviationWeatherSchema = {
  icaoCodes: z.array(z.string()).min(1).max(10).describe("Array of ICAO airport codes (e.g., [\"KJFK\", \"KLAX\", \"EGLL\"]) - minimum 1, maximum 10 codes. Case-insensitive."),
  includeTaf: z.boolean().default(false).describe("Include TAF (Terminal Aerodrome Forecast) data along with METAR. Default: false."),
};
...
export function registerSimBriefTools(server: McpServer, env: Env, props: Props): void {
  ...
  // Resolve SimBrief User ID from authenticated email
  const userId = resolveSimBriefUserId(props.email);
  if (!userId) {
    console.error(`No SimBrief User ID configured for ${props.email}`);
    throw new Error(`No SimBrief User ID configured for ${props.email}. Contact the server administrator.`);
  }
  console.log(`Resolved SimBrief User ID: ${userId} for ${props.email}`);
  ...
  server.tool(
    ...
    wrapWithSentry("getLatestFlightPlan", async () => {
      ...
      if (result.isError) {
        return {
          content: [
            {
              type: "text",
              text: `**Error**\n\n${result.error}\n\nPlease verify:\n1. The authenticated user (${props.email}) has an active flight plan on SimBrief\n2. The SimBrief API is accessible`,
              isError: true,
            },
          ],
        };
  ...
  server.tool(
    ...
    wrapWithSentry("getFlightPlanById", async ({ planId }) => {
  ...
  server.tool(
    ...
    wrapWithSentry("getDispatchBriefing", async () => {
      const result = await fetchSimBriefFlightPlan({ userId, apiKey });
...
*For complete details, use getLatestFlightPlan*`;
  ...
  server.tool(
    ...
    GetLatestFlightPlanSchema,
    wrapWithSentry("getLatestFlightPlanSummary", async () => {
      const result = await fetchSimBriefFlightPlan({ userId, apiKey });
  ...
  server.tool(
    ...
    GetFlightPlanByIdSchema,
    wrapWithSentry("getFlightPlanByIdSummary", async ({ planId }) => {
      const result = await fetchSimBriefFlightPlan({ userId, planId, apiKey });
  ...
  server.tool(
    ...
    GetNotamsSchema,
    wrapWithSentry("getNotams", async ({ airport = "all" }) => {
      const result = await fetchSimBriefFlightPlan({ userId, apiKey });
...
`;
  ...
  // Register aviation weather tool (METAR + optional TAF)
  server.tool(
    "getAviationWeather",
    "Retrieve current METAR (Meteorological Aerodrome Report) weather observations for specified airports. Returns raw METAR string, flight category (VFR/MVFR/IFR/LIFR), temperature, wind, and visibility. Optionally include TAF (Terminal Aerodrome Forecast) for weather predictions. Supports 1-10 ICAO codes per request. ICAO codes are case-insensitive. Use this when you need real-time aviation weather data.",
    GetAviationWeatherSchema,
    wrapWithSentry("getAviationWeather", async ({ icaoCodes, includeTaf }) => {
      try {
        const result = await getAviationWeather(icaoCodes, includeTaf);
        const markdown = formatAviationWeatherMarkdown(result);
        ...
        if (errorMessage.includes("non-empty array") || errorMessage.includes("Invalid ICAO")) {
          errorCode = "INVALID_INPUT";
        } else if (errorMessage.includes("Maximum 10")) {
          errorCode = "TOO_MANY_CODES";
        } else if (errorMessage.includes("timed out")) {
          errorCode = "UPSTREAM_UNAVAILABLE";
        }

        const errorResponse = createAviationWeatherErrorResponse(errorCode, errorMessage);
